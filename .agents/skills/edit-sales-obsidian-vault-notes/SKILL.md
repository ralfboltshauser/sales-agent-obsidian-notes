...
- **`Outreach Sends`** — YAML **`template:`** must include at least one `[[Outreach Templates/…]]` wikilink (`pnpm run validate:outreach-sends`). Optional **`subject`** / **`body`** hold the sendable copy for that thread.
