...
- **`Outreach Sends/`** — one note per send/thread; link **`people`**, optional **`message`** → template used.
...
5. **Draft in `Outreach Sends/`** — body or linked structure per existing `OutreachSend` examples; keep `sent_at` null until sent.
...
- Draft is in **`Outreach Sends/`** and linked from **`People.outreach_sends`**.
