...
5. **Outreach Sends:** every new or edited send must have YAML **`template:`** with at least one `[[Outreach Templates/...]]` link (enforced by `pnpm run validate:outreach-sends`—see [outreach-messages.md](outreach-messages.md)). Put sendable copy in **`subject`** / **`body`** when drafting or after send.
